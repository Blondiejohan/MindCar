    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.devEditCommand:
                ...
                for (int i = 0; i <= databaseAccess.getNumberOfRows("commands"); i++) {
                ...
                commandList.setAdapter(new SimpleAdapter(this, commands, android.R.layout.simple_list_item_2, new String[]{TEXT1, TEXT2}, new int[]{android.R.id.text1, android.R.id.text2}));

                break;
            case R.id.devEditDatabase:
                setContentView(R.layout.dev_edit_database);
                databaseAccess = DatabaseAccess.getInstance(this);
                databaseAccess.open();
                ...
                for (int i = 0; i <= databaseAccess.getNumberOfRows("users"); i++) {
                    Cursor cursor = databaseAccess.getRow("users", i);
                    data.put(TEXT1, cursor.getString(cursor.getColumnIndex("username")));
